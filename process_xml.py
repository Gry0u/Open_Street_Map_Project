...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS, default_tag_type='regular'):
    ...
    if element.tag == 'node':
        #handle separately the weird nove '136 rue masson'
        if element.get('id')=='4051563898':
            #add 'addr:house' tag:
            tags.append({'id':element.get('id'),'key':'housenumber','value':'136','type':'addr'})
...
expected_st_types=['Rue','Boulevard','Square','Place','Avenue','Jardin','Cours','Quai','Parking',\
                   'Eglise','Halles','Rampe','Impasse','Chemin','Route','Rond-Point','Traverse','Venelle','Port','La']
phone_re=re.compile(r'\+33\s\d(\s\d\d){4}')
    
    
mapping={'136':'',
         'Ernestine':'Rue Ernestine',
         'Albert':'Rue Albert',
         'C.CIAL':'Centre Commercial',
         'Kerlaurent':'Rue de Kerlaurent',
         'AV':'Avenue',
         'avenue':'Avenue',
         'Bd':'Boulevard',
         'bd':'Boulevard',
...
def clean(k,v):
    if k=='addr:street':
        return clean_street(v)
    elif (':phone' in k) or (':fax' in k):
...
def clean_street(name):
    ...
    if street_type in mapping.keys():
        ...
        repl=street_type
    return re.sub(street_type_re,repl,name,1).strip()     
